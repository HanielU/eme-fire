<script lang="ts">
  ...
  import type { ActionsPageRouterStore, MapDetailsSearchStore } from "../../../utils/utils";
  ...
  function returnPercent(initialRideTime: number): number {
    let percent: number = +((initialRideTime * 100) / rideDurationInSecs).toFixed(2);
...
<div class="fire-service page-wrapper" transition:fly={{ x: 300, duration: 250 }}>
  <h1>
  ...
  <h4 class="duration__timer">
    {serviceHired ? (rideStarted ? `Distance Covered ${ridePercent}%` : "Ride Done!") : ""}
  </h4>
</div>
